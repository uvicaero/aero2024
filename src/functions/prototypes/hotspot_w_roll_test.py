...
# Assumed global calibration variables (you need to define these appropriately)
# Example:
# camera_matrix = np.array([[fx, 0, cx],
#                           [0, fy, cy],
#                           [0,  0,  1]])
# dist_coeffs = np.array([k1, k2, p1, p2, k3])
# img_half_height = 2464 // 2
# img_half_width = 3280 // 2
# phi_y = 0.001  # example value, adjust accordingly
# long_factor = 1e5  # example conversion factor
# lat_factor = 1e5   # example conversion factor
...
def get_hotspots_gps(distorted_points, cam_x, cam_y, altitude, pitch, azimuth, roll):
    ...
    points = cv2.undistortImagePoints(dist_pts, camera_matrix, dist_coeffs)
    projected = np.zeros(points.shape, np.float64)

    i = 0
    for point in points:
        # Compute fraction of pixels offset from center
        frac_pixels_y = -(point[0][1] - img_half_height) / img_half_height  
        frac_pixels_x = (point[0][0] - img_half_width) / img_half_width

        # Apply roll correction using a 2D rotation matrix
        cos_roll = math.cos(roll)
        sin_roll = math.sin(roll)
        corrected_x = frac_pixels_x * cos_roll - frac_pixels_y * sin_roll
        corrected_y = frac_pixels_x * sin_roll + frac_pixels_y * cos_roll

        # Compute `ground_k` (forward ground distance)
        ground_k = altitude * math.tan(pitch + corrected_y * phi_y)

        # Compute the ground displacement angle
        ground_angle = math.atan2(corrected_x, -corrected_y)  
...
def generate_and_upload_kml(capture_spots, detected_hotspots, timestamp, rel_alt):
    ...
    for i, point in enumerate(capture_spots, start=1):
        pnt = kml.newpoint(name=f"Capture point {i}", coords=[(point["lon"], point["lat"], 0)])
    ...
    for marker_name, coords in detected_hotspots.items():
        pnt = kml.newpoint(name=f"Detected Hotspot {detected_hotspot_count}", coords=[(coords["lon"], coords["lat"], 0)])
...
def main():
    ...
    # Define movements for each direction along with return offsets and sleep flags
    movements = [
        {"direction": "North", "move": (5, 0, 0),   "return": (-5, 0, 0), "sleep_after": False},
        {"direction": "East",  "move": (0, 5, 0),   "return": (0, -5, 0), "sleep_after": False},
        {"direction": "South", "move": (-5, 0, 0),  "return": (5, 0, 0),  "sleep_after": True},
        {"direction": "West",  "move": (0, -5, 0),  "return": (0, 5, 0),  "sleep_after": True},
    ]
    ...
    for move in movements:
        ...
        print(f"\nPress Enter to move {direction} (5m)...")
        input()
        send_body_offset_local_position(the_connection, *move["move"])
        ...
        plt.legend()
        plt.title(f"Hotspots Detected in Image {image_count}")
        plt.show()
        
        # Map hotspots to GPS coordinates using the new `get_hotspots_gps` function
        if hotspots.size > 0:
            distorted_points = np.array([[x, y] for x, y in hotspots], dtype=np.float32)
            camera_pitch = pitch - math.radians(90)
            roll = 0  # Default roll value; adjust if you have roll data
            gps_hotspots = get_hotspots_gps(distorted_points, lon, lat, rel_alt, camera_pitch, azimuth, roll)
            ...
            if gps_hotspots.size > 0 and gps_hotspots.shape[1] == 2:
                for j, gps_point in enumerate(gps_hotspots):
                    hotspot_name = f"Hotspot_{direction}_{j+1}"
        ...
        print(f"Image {image_count} processed.\n")
        image_count += 1
        
        # Return to original position
        send_body_offset_local_position(the_connection, *move["return"])
        if move["sleep_after"]:
            time.sleep(5)
    ...
    # Generate and upload the KML file using the separate function
    generate_and_upload_kml(capture_spots, detected_hotspots, timestamp, rel_alt)
