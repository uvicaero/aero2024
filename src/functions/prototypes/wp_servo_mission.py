...
def create_mission_waypoints(coordinates_dict):
    """
    Create waypoints for the mission based on the pattern provided, including an autocontinue field.
    Also, return waypoint indices where autocontinue = 0.
    ...
    # Define reservoir waypoints
    reservoir_lat, reservoir_lon = reservoir['latitude'], reservoir['longitude']
    reservoir_waypoints = [
        {'lat': reservoir_lat, 'lon': reservoir_lon, 'alt': 20, 'autocontinue': 1},  # Reservoir at 20m
        {'lat': reservoir_lat, 'lon': reservoir_lon, 'alt': 5, 'autocontinue': 1},   # Descend to 5m
        {'lat': reservoir_lat, 'lon': reservoir_lon, 'alt': 10, 'autocontinue': 1},  # Ascend to 10m
    ]

    # Add reservoir start waypoints
    waypoints.extend(reservoir_waypoints)
    
    # Loop through each bucket and add the pattern
    for bucket_name, bucket_coords in sorted_buckets:
        bucket_lat, bucket_lon = bucket_coords['latitude'], bucket_coords['longitude']
        
        # Waypoints for the current bucket
        bucket_waypoints = [
            {'lat': bucket_lat, 'lon': bucket_lon, 'alt': 20, 'autocontinue': 1},  # Fly to bucket at 20m
            {'lat': bucket_lat, 'lon': bucket_lon, 'alt': 5, 'autocontinue': 0},   # Descend to 5m (no autocontinue)
            {'lat': bucket_lat, 'lon': bucket_lon, 'alt': 10, 'autocontinue': 1},  # Ascend to 10m
        ]
        
        # Append waypoints and record indices with autocontinue = 0
        waypoints.extend(bucket_waypoints)
        autohold_indices.append(len(waypoints) - 2)  # Record index for the 5m waypoint (autocontinue = 0)
        
        # Return to reservoir at 20m
        return_to_reservoir = {'lat': reservoir_lat, 'lon': reservoir_lon, 'alt': 20, 'autocontinue': 1}
        waypoints.append(return_to_reservoir)
...
def main():
    ...
    while True:
        ...
        set_servo_pwm(the_connection, 9, 1100)
        time.sleep(5)
        resume_mission(the_connection)
        time.sleep(5)
        set_servo_pwm(the_connection, 9, 1500)
