...
# Return: returns the desired location of the bucket as a tuple (x_desired, y_desired).
def determine_desired_location(image_shape, camera_offset, bucket_real_radius, altitude, fov, detected_bucket_radius, scale_factor):
    ...
    # Compute an average pixels per meter conversion factor (this is done because there is just one value for the detected radius (no x and y))
...
def displacement_in_meters(detected_location, image_shape, camera_offset, bucket_real_radius, altitude, fov, detected_bucket_radius, scale_factor):
    ...
    displacement_x_pixels = desired_location[0] - detected_location[0] 
...
def displayAverage(circle_params):
    ...
    altitude = 0.30  # Drone altitude in meters
